...
<table width="100%" height="100%" border="0" cellpadding="0" cellspacing="0" style="border-left:1px solid #B7E6B0;border-right:1px solid #B7E6B0;">
  <tr>
    <td height="30" background="images/main_title_bg.jpg">&nbsp;&nbsp;您的当前位置：<a href="#">系统首页</a> > 资讯管理</td>
  </tr>
  <tr>
...
{
	$cat_idss = "";
	$sql = "select * from wkcx_class where pid='$cat_id'";
	$res = $db->query($sql);
...
$countSql="SELECT a.id FROM wkcx_content AS a,wkcx_class AS b WHERE a.cat_id=b.id  $sqlwhere  AND b.type='news'";//sql语句
...
$sql = "SELECT a.id AS id,news_title,class_name,add_time,news_order,img_thumb FROM wkcx_content AS a,wkcx_class AS b WHERE a.cat_id=b.id  $sqlwhere  AND b.type='news' ORDER BY news_order ASC,add_time DESC LIMIT $rsStart, $pageSize";
$res = $db->query($sql);
$i = 1;
while($row = $db->getarray($res))
{
