...
<?php
	...
	{
		$sql = "select * from wkcx_left where is_show=1 order by menu_order asc,id asc";
	}
	else
	{
		$sql_user = "select * from wkcx_admin as a,wkcx_jiaose as b where a.admin_type=b.id and admin_name='".$_SESSION['username']."'";
		...
		$sql = "select * from wkcx_left where id in($all) order by menu_order asc,id asc";
	}
	$res = $db->query($sql);
	...
	<tr>
		...
		<table width="120" height="31" border="0" cellpadding="0" cellspacing="0">
          <tr>
            ...
            <td align="left"><p style="margin-top:4px"><a href="<?=$row['url']?>" target="main-frame"><?=$row['name']?></a></p></td>
          </tr>
        </table>
        </td>
      </tr>
