<?php 
	...
	$id = !empty($_GET['id']) ? intval($_GET['id']) : '';
		...
		$sql = "SELECT * FROM wkcx_admin WHERE id='$id'";
		$res = $db->query($sql);
		$row = $db->getarray($res);
          ...
          <table width="100%" border="0" align="center" cellpadding="5" cellspacing="1" bgcolor="#FF9900">
            <tr>
             ...
             <tr>
              ...
              <?php
              	$sql = "select * from wkcx_jiaose order by id asc";
				$res = $db->query($sql);
...
<?php
	if($act == 'update')
	{
		...
		$sql = "UPDATE wkcx_admin SET admin_pwd='$password2',admin_type='$admin_type' WHERE admin_name='$username'";
		$db->query($sql);
		goBakLoad('修改成功');
