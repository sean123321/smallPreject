...
<table width="100%" height="100%" border="0" cellpadding="0" cellspacing="0" style="border-left:1px solid #B7E6B0;border-right:1px solid #B7E6B0;">
            ...
            <tr>
              <td height="30" align="right"><table width="70" height="22" border="0" cellpadding="0" cellspacing="0" bgcolor="#009900" onMouseOver="this.style.backgroundColor='#003300';" onMouseOut="this.style.backgroundColor='#009900';">
                <tr>
                  ...
                  <td align="center"><p style="margin-top:3px"><span onClick="sAlertopen('656','217','class_add.php');" style="font-size:12px; font-weight:normal; color:#FFFFFF; cursor:hand">添加类别</span></p></td>
                </tr>
              </table></td>
              </tr>
  ...
  <TBODY>
  <TR>
    <TH width="29" background="images/list_title_bg.jpg">ID</TH>
    <TH width="1119" height="24" background="images/list_title_bg.jpg">栏目名称</TH>
    
    <TH width="65" background="images/list_title_bg.jpg">模型</TH>
    <TH width="65" background="images/list_title_bg.jpg">排序</TH>
    <TH width="88" align="center" background="images/list_title_bg.jpg">操作</TH>
  </TR>
...
?>   
   ...
   <TD align="center"><?=$row['id']?></TD>
    ...
    <TD align="center"><?php if($row['model']=='1'){echo "单篇";}elseif($row['model']=='2'){echo "文字列表";}elseif($row['model']=='3'){echo "图片列表";}elseif($row['model']=='4'){echo "图文列表";}elseif($row['model']=='5'){echo "图文轮换";}?></TD>
    <TD align="center"><?=$row['class_order']?></TD>
    <TD align=center>
    <table width="60" border="0" cellspacing="0" cellpadding="0">
      <tr>
	   <td align="center"><span style="cursor:hand;color:#ff0000" onClick="sAlertopen('407','200','add_pic.php?id=<?=$row['id'];?>&class_name=<?=urlencode($row['class_name'])?>')" ><img src="images/image.gif" width="16" height="16"></span></td>
	  
        <td align="center"><span onClick="sAlertopen('656','249','class_upd.php?id=<?=$row['id']?>&act=upd');" style="font-size:12px; font-weight:normal; color:#FFFFFF; cursor:hand"><img src="images/icon_edit.gif" width="16" height="16" border="0" alt="修改"></span></td>
...
<?php
	...
	if($act == 'del')
	{
		...
		$sql = "SELECT id FROM wkcx_class WHERE pid='$id'";
		if($db->getcount($sql)>0)
		{
			goBakMsg('该分类下还有下级分类，请先删除下级分类');
			exit();
		}
		$sql = "SELECT id FROM wkcx_content WHERE cat_id='$id'";
		if($db->getcount($sql)>0)
		{
			goBakMsg('该分类下存在信息，请删除该分类的信息后再删除该分类');
			exit();
		}
		$sql = "DELETE FROM wkcx_class WHERE id='$id'";
		$db->query($sql);
