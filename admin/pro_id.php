<?php 
	...
	$keywords = !empty($_GET['keywords']) ? trim($_GET['keywords']) : '请输入产品ID号';
?>
...
<table width="100%" height="100%" border="0" cellpadding="0" cellspacing="0" style="border-left:1px solid #B7E6B0;border-right:1px solid #B7E6B0;">
  <tr>
    <td height="30" background="images/main_title_bg.jpg">&nbsp;&nbsp;您的当前位置：<a href="#">系统首页</a> > 产品ID管理</td>
  </tr>
  <tr>
    <td align="center" valign="top"><table width="100%" border="0" cellspacing="0" cellpadding="0">
      ...
      <tr>
        ...
        <option value="">请选择产品分类</option>
              ...
              <td width="558" align="center"><input name="keywords" type="text" value="<?=$keywords?>" style="width:100%" class="inputuser" onBlur="this.className='inputuser'" onFocus="this.className='inputuser-bor'; this.value=''"></td>
              <td width="265"><input name="Submit" type="submit" class="button" value="立即搜索" onMouseMove="this.className='button-bor'" onMouseOut="this.className='button'" ></td>
              <td width="360" align="right"><table width="90" height="22" border="0" cellpadding="0" cellspacing="0" bgcolor="#009900" onMouseOver="this.style.backgroundColor='#003300';" onMouseOut="this.style.backgroundColor='#009900';">
                  <tr>
                    <td align="center"><img src="images/icon_add.gif" width="16" height="16"></td>
                    <td align="center"><span onClick="sAlertopen('300','185','proid_add.php');" style="font-size:12px; font-weight:normal; color:#FFFFFF; cursor:hand">添加产品ID</span></td>
                  </tr>
              </table></td>
            </tr></form>
          </table>
        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="0">
          ...
          <table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#BCE8B5">
            <tr>
              ...
              <td align="center" background="images/list_title_bg.jpg"><strong>产品ID</strong></td>
              <td width="100" align="center" background="images/list_title_bg.jpg"><strong>产品分类</strong></td>
...
if(!empty($keywords)&&$keywords!='请输入产品ID号')
{
$sqlwhere.=" AND a.bianhao='$keywords'";

}
$countSql="SELECT a.id FROM wkcx_yz  AS a,wkcx_class AS b WHERE a.cat_id=b.id $sqlwhere ";//sql语句
...
$sql = "SELECT a.id AS id,bianhao,class_name  FROM wkcx_yz  AS a,wkcx_class AS b WHERE a.cat_id=b.id $sqlwhere   ORDER BY id ASC  LIMIT $rsStart, $pageSize";
$res = $db->query($sql);
$i = 1;
...
?>            
            <tr <?=$style?>>
              ...
              <td align="left"><?=$row['bianhao']?></td>
              <td align="center"><?=$row['class_name']?></td>
              <td align="center"><table width="60" border="0" cellspacing="0" cellpadding="0">
                <tr>
                  <td align="center">
				  <span onClick="sAlertopen('300','185','proid_upd.php?act=upd&id=<?=$row['id']?>&curPage=<?=$curPage?>&<?=$KeyWord?>');" style="font-size:12px; font-weight:normal; color:#FFFFFF; cursor:hand"> <img src="images/icon_edit.gif" width="16" height="16" border="0" alt="修改"></span>
...
$url = "pro_id.php?curPage=".$curPage."&".$urlPara."";
if($act == 'del'){
	$id = !empty($_GET['id']) ? intval($_GET['id']) : '';
	$sql = "DELETE FROM wkcx_yz WHERE id='$id'";
	$db->query($sql);
	urlMsg('删除成功',$url);
}
//文章全部删除
if($act=='delAll'){
	...
	if(!empty($bh)){
		foreach($bh as $key =>$value){
			echo $sql = "DELETE FROM wkcx_yz WHERE id='$value'";
			$db->query($sql);
		}
		urlMsg('删除成功',$url);
	}
