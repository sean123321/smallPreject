...
<table width="100%" height="100%" border="0" cellpadding="0" cellspacing="0" style="border-left:1px solid #B7E6B0;border-right:1px solid #B7E6B0;">
  <tr>
    <td height="30" background="images/main_title_bg.jpg">&nbsp;&nbsp;您的当前位置：<a href="#">系统首页</a> > 会员管理</td>
  </tr>
  <tr>
        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="0">
          ...
          <table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#BCE8B5">
            <tr>
              ...
              <td width="100" align="center" background="images/list_title_bg.jpg"><strong>用户名</strong></td>
              <td width="120" align="center" background="images/list_title_bg.jpg"><strong>姓名</strong></td>
              ...
              <td width="80" align="center" background="images/list_title_bg.jpg"><strong>会员类别</strong></td>
...
{
	$cat_idss = "";
	$sql = "select * from wkcx_user  where typeid =$cat_id";
	$res = $db->query($sql);
...
{
	$sqlwhere.=" AND user_name LIKE '%$keywords%' or name LIKE '%$keywords%'";
}
...
$countSql="SELECT * FROM wkcx_user 1=1";//sql语句
...
$sql = "SELECT * FROM wkcx_user where 1=1 $sqlwhere ORDER BY add_time desc LIMIT $rsStart, $pageSize";
$res = $db->query($sql);
$i = 1;
while($row = $db->getarray($res))
{
...
?>            
            <tr <?=$style?>>
              ...
              <td align="center"><?php if($row['sex']==1){echo "男";}elseif($row['sex']==0){echo "女";}?></td>
              <td align="center"><?php if($row['typeid']==1){echo "正式会员";}elseif($row['typeid']==2){echo "普通会员";}?></td>
              <td align="center"><?=$row['add_time']?></td>
              <td align="center"><table width="60" border="0" cellspacing="0" cellpadding="0">
                <tr>
                  <td align="center">
                  
                  <span onClick="sAlertopen('550','450','user_lookup.php?id=<?=$row['id']?>&act=look');" style="font-size:12px; font-weight:normal; color:#FFFFFF; cursor:hand"><img src="images/icon_view.gif" width="16" height="16"  border="0"></span></td>
...
if($act == 'del'){
	...
	$sql = "DELETE FROM wkcx_user WHERE id='$id'";
	$db->query($sql);
	urlMsg('删除成功',$url);
}
//文章全部删除
if($act=='delAll'){
	...
	if(!empty($bh)){
		foreach($bh as $key =>$value){
			echo $sql = "DELETE FROM wkcx_user WHERE id='$value'";
			$db->query($sql);
		}
		urlMsg('删除成功',$url);
	}
