...
$id_url = !empty($_GET['id']) ? intval($_GET['id']) : '';
	$sql = "select * from wkcx_xiangce  where red_id='$id_url'";
	$res = $db->query($sql);
...
if($act == 'del')
{
	...
	$sql_sel = "select * from wkcx_xiangce where id='$id_del'";
	$res = $db->query($sql);
	$row_u = $db->getarray($res);
	@unlink($row_u['file_url']);
	$sql = "delete from wkcx_xiangce where id='$id_del'";
	if($db->query($sql))
	{
	
	$sql = "SELECT * FROM wkcx_xiangce    WHERE img_bthumb!=''  and red_id='$id_url' ORDER BY  add_time DESC ";
