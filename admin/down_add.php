...
<DIV class="tagContent selectTag" id=tagContent0>
  <table width="100%" border="0" cellpadding="5" cellspacing="1" bgcolor="#CCCCCC">
    ...
    <tr>
      ...
      <td bgcolor="#FFFFFF"><select name="cat_id" id="select">
        ...
        <?php
                    ...
                    {
                        $sql = "SELECT * FROM wkcx_class WHERE pid='$pid' and type='down' ORDER BY class_order ASC,id DESC";
                        $res = mysql_query($sql);
    ...
    <tr>
      <td align="right" bgcolor="#FFFFFF"><strong>文件附件：</strong></td>
      <td bgcolor="#FFFFFF"><!--<input type="file" name="img_thumb" id="img_thumb">-->
	  <input name="down_src" type="text" style="width:450px;" class="inputuser" onBlur="this.className='inputuser'" onFocus="this.className='inputuser-bor'">
	  </td>
      </tr>
    ...
    <tr>
      <td align="right" bgcolor="#FFFFFF"><strong>下载简介：</strong></td>
...
<?php
	...
	if($act == 'add')
	{
		...
		/*if(!empty($_FILES['img_thumb']['name']))
		{
			...
			$img_thumb = !empty($_POST['down_src']) ? $_POST['down_src'] : '';
		}*/
		
		$img_thumb1 = !empty($_POST['down_src']) ? $_POST['down_src'] : '';
		$img_thumb = "file/".$img_thumb1;
		
		$sql = "INSERT INTO wkcx_content(xiazaiid,cat_id,news_title,news_desc,news_content,add_time,img_thumb,news_url,type,news_order,small_img) 
						VALUES('$xiazai','$cat_id','$news_title','$news_desc','$news_content','$add_time','$img_thumb','$news_url','1','$news_order','$small_img')";
		//exit;
		if($db->query($sql))
		{
			urlMsg('添加成功','down_list.php');
		}
		else
		{
			goBakMsg("添加失败");	
