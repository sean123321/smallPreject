...
$countSql="SELECT id FROM wkcx_data $sqlwhere";//sql语句
$KeyWord = "cat=$cat";
$pageSize="20"; //每页显示数
...
$sql = "SELECT * FROM wkcx_data $sqlwhere ORDER BY id DESC LIMIT $rsStart, $pageSize";
$res = $db->query($sql);
$i = 1;
...
if($act == 'del'){
	...
	$sql = "DELETE FROM wkcx_data WHERE id='$id'";
	$db->query($sql);
	urlMsg('删除成功',$url);
}
//全部删除
if($act=='delAll'){
	...
	if(!empty($bh)){
		foreach($bh as $key =>$value){
			echo $sql = "DELETE FROM wkcx_data WHERE id='$value'";
			$db->query($sql);
		}
		urlMsg('删除成功',$url);
	}
