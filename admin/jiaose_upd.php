<?php 
	...
	$sql = "select * from wkcx_jiaose where id='$id'";
	$res = $db->query($sql);
          ...
          <table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#BCE8B5">
            ...
            <tr>
              <td height="30" colspan="2" align="left" bgcolor="#FFFFFF" ><p><strong>栏目权限</strong></p>
                <p>
                <?php
                	$sql = "select * from wkcx_left order by id asc";
					$res = $db->query($sql);
...
<?php
	...
	if($act == 'add')
	{
		...
		$qx = !empty($_POST['qx']) ? serialize($_POST['qx']) : '';
		 $sql = "update wkcx_jiaose set jiaose_name='$jiaosename',jiaose_quanxian='$qx',jiaose_left='$left' where id='$id'";
